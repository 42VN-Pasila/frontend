...
const DataGridContext = createContext<DataDisplayGridContextValue<unknown> | null>(null);

function useDataDisplayGridContext<T>() {
    const ctx = useContext(DataGridContext);
    if (!ctx) throw new Error("DataDisplayGrid components must be used inside <DataDisplayGrid />");
...
interface DataDisplayGridRootProps<T> extends React.HTMLAttributes<HTMLDivElement> {
    items: T[];
    itemToText?: (item: T) => string;
...
type SearchProps = Omit<React.InputHTMLAttributes<HTMLInputElement>, "value" | "onChange"> & {
    wrapperClassName?: string;
};

const Search = ({ className, wrapperClassName, placeholder = "Search...", ...rest }: SearchProps) => {
